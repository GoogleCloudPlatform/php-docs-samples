...
class tasksTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        if (!self::$hasCredentials) {
            ...
            $this->markTestSkipped('No Pull Queue was found.');
    ...
    {
        $output = $this->runCommand('pull-and-acknowledge-task', ['project' => self::$project, 'queue' => self::$queue, 'location' => self::$location]);
        $taskNamePrefix = sprintf('projects/%s/locations/%s/queues/%s/tasks/',
            self::$project,
            self::$location,
            self::$queue
